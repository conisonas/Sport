    ...
    <div class="p-8 flex items-start bg-light-grey rounded-md shadow-lg">
      ...
      <form @submit.prevent="createWorkout" class="flex flex-col gap-y-5 w-full">
        ...
        <div class="flex flex-col">
          ...
          <select id="workout-type" class="p-2 text-gray-500 focus:outline-none" required @change="workoutChange"
            v-model="workoutType">
      ...
      hover:text-at-light-green">
        ...
        <div v-if="workoutType === 'cardio'" class="flex flex-col gap-y-4">
          <div class="flex flex-col gap-x-6 gap-y-2 relative md:flex-row" v-for="(item, index) in exercises"
            :key="index">
            <div class="flex flex-col md:w-1/3">
              <label for="cardio-type" class="mb-1 text-sm text-at-light-green">类型
              </label>
            ...
            <div class="flex flex-col flex-1">
              <label for="distance" class="mb-1 text-sm text-at-light-green">距离
              </label>
...
<script setup>
  ...
  const deleteExercise = (id) => {
    ...
    errorMsg.value = "错误:不能移除,至少需要一个运动项目";
    setTimeout(() => {
  ...
  const createWorkout = async () => {
    try {
      ...
      statusMsg.value = "运动已创建!";
      workoutName.value = null;
      workoutType.value = "选择运动类型";
      exercises.value = [];
      setTimeout(() => {
